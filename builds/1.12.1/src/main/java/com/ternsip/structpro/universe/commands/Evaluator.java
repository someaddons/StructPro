...
class Evaluator {
    ...
    static String cmdPaste(UWorld uWorld,
        ...
        if (isVillage) {
            ...
            Report report = projection.project(isInsecure);
            report.print();
            uWorld.sound(new UBlockPos(posX, posY, posZ), SoundEvents.BLOCK_GLASS_PLACE, SoundCategory.BLOCKS, 1.0f);
            return report.toString();
        }
    }
    ...
    static String cmdSave(UWorld uWorld, String name, int posX, int posY, int posZ, int width, int height, int length) {
        ...
        report.print();
        uWorld.sound(new UBlockPos(posX, posY, posZ), SoundEvents.BLOCK_ENDERCHEST_CLOSE, SoundCategory.BLOCKS, 0.5f);
        return report.toString();
    }
